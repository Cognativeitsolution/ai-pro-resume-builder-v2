"use client";
import { addUserSummary, removeSection } from '@/redux/slices/addSectionSlice';
...
const AllSummary = ({ textValue = '', data = {} }: AllSummaryType) => {
    const dispatch = useDispatch();

    const [inputSkill, setInputSkill] = useState<string>('');
    const [allSummaryData, setAllSummaryData] = useState<string[]>([]);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
                ...
                if (allSummaryData.length > 0 && data?.id) {
                    const SummaryPayload = allSummaryData.map(skill => ({
                        type: "skill",
                        name: skill
                    }));
    ...
    }, [allSummaryData, data?.id, dispatch]);

    const handleAddSkill = () => {
        if (inputSkill.trim() !== '') {
            setAllSummaryData([...allSummaryData, inputSkill.trim()]);
            setInputSkill('');
            setShowInput(false);
        }
    };
    ...
    const handleDeleteSkill = (index: number) => {
        const updated = allSummaryData.filter((_, i) => i !== index);
        setAllSummaryData(updated);
    };

    const handleEditSkill = (index: number) => {
        setEditingIndex(index);
        setEditedSkill(allSummaryData[index]);
    };

    const handleSaveEdit = () => {
        if (editingIndex !== null && editedSkill.trim() !== '') {
            const updated = [...allSummaryData];
    ...
    return (
        ...
        <div ref={containerRef}
            ...
            <div className="mt-1 flex flex-wrap gap-2">
                {allSummaryData?.length ? allSummaryData.map((skill, index) => (
                    <div
                        key={index}
                    ...
                    <span onClick={handleShowInput} className='text-gray-300'>Add Summary</span>
                }
            </div>
